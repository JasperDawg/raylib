 ...
 *           If defined, the library can be used as standalone as a camera
 *system but some functions must be redefined to manage inputs accordingly.
 *
 *   CONTRIBUTORS:
 ...
 *   LICENSE: zlib/libpng
 *
 *   Copyright (c) 2022-2025 Christoph Wagner (@Crydsch) & Ramon Santamaria
 *(@raysan5)
 *
...
typedef struct Camera3D {
  ...
  int projection; // Camera projection type: CAMERA_PERSPECTIVE or
                  // CAMERA_ORTHOGRAPHIC
} Camera3D;
...
// Camera movement
RLAPI void CameraMoveForward(Camera *camera, float distance,
                             bool moveInWorldPlane);
RLAPI void CameraMoveUp(Camera *camera, float distance);
RLAPI void CameraMoveRight(Camera *camera, float distance,
                           bool moveInWorldPlane);
RLAPI void CameraMoveToTarget(Camera *camera, float delta);

// Camera rotation
RLAPI void CameraYaw(Camera *camera, float angle, bool rotateAroundTarget);
RLAPI void CameraPitch(Camera *camera, float angle, bool lockView,
...
void CameraMoveToTarget(Camera *camera, float delta) {
  ...
  Vector3 forward = GetCameraForward(camera);
  camera->position =
      Vector3Add(camera->target, Vector3Scale(forward, -distance));
}
...
//  - rotateAroundTarget defines if rotation is around target or around its
//  position
//  - rotateUp rotates the up direction as well (typically only usefull in
//  CAMERA_FREE)
// NOTE: angle must be provided in radians
void CameraPitch(Camera *camera, float angle, bool lockView,
                 bool rotateAroundTarget, bool rotateUp) {
  // Up direction
  Vector3 up = GetCameraUp(camera);
...
Matrix GetCameraProjectionMatrix(Camera *camera, float aspect) {
  if (camera->projection == CAMERA_PERSPECTIVE) {
    return MatrixPerspective(camera->fovy * DEG2RAD, aspect,
    ...
    return MatrixOrtho(-right, right, -top, top, CAMERA_CULL_DISTANCE_NEAR,
                       CAMERA_CULL_DISTANCE_FAR);
  }

  return R_MatrixIdentity();
}

#if !defined(RCAMERA_STANDALONE)
// Update camera position for selected mode
// Camera mode: CAMERA_FREE, CAMERA_FIRST_PERSON, CAMERA_THIRD_PERSON,
// CAMERA_ORBITAL or CUSTOM
void UpdateCamera(Camera *camera, int mode) {
  ...
  if (mode == CAMERA_CUSTOM) {
    ...
    if (IsKeyDown(KEY_DOWN))
      CameraPitch(camera, -cameraRotationSpeed, lockView, rotateAroundTarget,
                  rotateUp);
    if (IsKeyDown(KEY_UP))
      CameraPitch(camera, cameraRotationSpeed, lockView, rotateAroundTarget,
                  rotateUp);
...
void UpdateCameraPro(Camera *camera, Vector3 movement, Vector3 rotation,
  ...
  // Camera rotation
  CameraPitch(camera, -rotation.y * DEG2RAD, lockView, rotateAroundTarget,
              rotateUp);
