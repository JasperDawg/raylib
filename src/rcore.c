/**********************************************************************************************
 *
 *   rcore - Window/display management, Graphic device/context management and
 *input management
 *
 *   PLATFORMS SUPPORTED:
 ...
 *           Default font is loaded on window initialization to be available for
 *the user to render simple text. NOTE: If enabled, uses external module
 *functions to load default raylib font (module: text)
 *
 *       #define SUPPORT_CAMERA_SYSTEM
 *           Camera module is included (rcamera.h) and multiple predefined
 *cameras are available: free, 1st/3rd person, orbital, custom
 *
 *       #define SUPPORT_GESTURES_SYSTEM
 *           Gestures module is included (rgestures.h) to support gestures
 *detection: tap, hold, swipe, drag
 *
 *       #define SUPPORT_MOUSE_GESTURES
 *           Mouse gestures are directly mapped like touches and processed by
 *gestures system.
 *
 *       #define SUPPORT_BUSY_WAIT_LOOP
 *           Use busy wait loop for timing sync, if not defined, a
 *high-resolution timer is setup and used
 *
 *       #define SUPPORT_PARTIALBUSY_WAIT_LOOP
 *           Use a partial-busy wait loop, in this case frame sleeps for most of
 *the time and runs a busy-wait-loop at the end
 *
 *       #define SUPPORT_SCREEN_CAPTURE
 *           Allow automatic screen capture of current screen pressing F12,
 *defined in KeyCallback()
 *
 *       #define SUPPORT_GIF_RECORDING
 *           Allow automatic gif recording of current screen pressing CTRL+F12,
 *defined in KeyCallback()
 *
 *       #define SUPPORT_COMPRESSION_API
 *           Support CompressData() and DecompressData() functions, those
 *functions use zlib implementation provided by stb_image and stb_image_write
 *libraries, so, those libraries must be enabled on textures module for linkage
 *
 *       #define SUPPORT_AUTOMATION_EVENTS
 *           Support automatic events recording and playing, useful for
 *automated testing systems or AI based game playing
 *
 *   DEPENDENCIES:
 *       raymath  - 3D math functionality (Vector2, Vector3, Matrix, Quaternion)
 *       camera   - Multiple 3D camera modes (free, orbital, 1st person, 3rd
 *person) gestures - Gestures system for touch-ready devices (or simulated from
 *mouse inputs)
 *
 *
 *   LICENSE: zlib/libpng
...
//----------------------------------------------------------------------------------
     ...
     defined(PLATFORM_WEB_RGFW)) &&                                            \
    (_XOPEN_SOURCE < 500)
#undef _XOPEN_SOURCE
#define _XOPEN_SOURCE                                                          \
  500 // Required for: readlink if compiled with c99 without gnu ext.
#endif
...
// Platform specific defines to handle GetApplicationDirectory()
    ...
    (defined(_MSC_VER) && defined(PLATFORM_DESKTOP_RGFW))
#ifndef MAX_PATH
#define MAX_PATH 1025
#endif
struct HINSTANCE__;
__declspec(dllimport) unsigned long __stdcall GetModuleFileNameA(
    struct HINSTANCE__ *hModule, char *lpFilename, unsigned long nSize);
__declspec(dllimport) unsigned long __stdcall GetModuleFileNameW(
    struct HINSTANCE__ *hModule, wchar_t *lpFilename, unsigned long nSize);
...
__declspec(dllimport) unsigned int __stdcall timeBeginPeriod(
    unsigned int uPeriod);
__declspec(dllimport) unsigned int __stdcall timeEndPeriod(
    unsigned int uPeriod);
...
#define MAX_FILEPATH_LENGTH                                                    \
  256 // On Win32, MAX_PATH = 260 (limits.h) but Windows 10, Version 1607
      // enables long paths...
#else
#define MAX_FILEPATH_LENGTH                                                    \
  4096 // On Linux, PATH_MAX = 4096 by default (limits.h)
#endif
#endif

#ifndef MAX_SCISSOR_STACK_SIZE

#define MAX_SCISSOR_STACK_SIZE                                                 \
  8 // Maximum number of nested scissor calls supported
...
#define MAX_GAMEPAD_NAME_LENGTH                                                \
  128 // Maximum number of characters in a gamepad name (byte size)
#endif
...
#define MAX_DECOMPRESSION_SIZE                                                 \
  64 // Maximum size allocated for decompression in MB
#endif
...
#define DIRECTORY_FILTER_TAG                                                   \
  "DIR" // Name tag used to request directory inclusion on directory scan
#endif  // NOTE: Used in ScanDirectoryFiles(), ScanDirectoryFilesRecursively()
        // and LoadDirectoryFilesEx()
...
typedef struct CoreData {
  struct {
    ...
    bool usingFbo; // Using FBO (RenderTexture) for rendering instead of default
                   // framebuffer

    Point position;         // Window position (required on fullscreen toggle)
    Point previousPosition; // Window previous position (required on borderless
                            // windowed toggle)
    ...
    Rectangle scissorStack[MAX_SCISSOR_STACK_SIZE]; // Scissor stack for nested
                                                    // scissor callsAdd
                                                    // commentMore actions
  ...
  struct {
    struct {
      ...
      char currentKeyState[MAX_KEYBOARD_KEYS];  // Registers current frame key
                                                // state
      char previousKeyState[MAX_KEYBOARD_KEYS]; // Registers previous frame key
                                                // state

      // NOTE: Since key press logic involves comparing prev vs cur key state,
      // we need to handle key repeats specially
    ...
    struct {
      ...
      char previousButtonState[MAX_GAMEPADS]
                              [MAX_GAMEPAD_BUTTONS]; // Previous gamepad buttons
                                                     // state
...
//----------------------------------------------------------------------------------
RLAPI const char *raylib_version =
    RAYLIB_VERSION; // raylib version exported symbol, required for some
                    // bindings
...
// NOTE: Useful to allow Texture, RenderTexture, Font.texture, Mesh.vaoId/vboId,
// Shader loading
...
static bool automationEventRecording =
    false; // Recording automation events flag
// static short automationEventEnabled = 0b0000001111111111; // TODO: Automation
// events enabled for recording/playing
#endif
//-----------------------------------------------------------------------------------
...
extern void
LoadFontDefault(void); // [Module: text] Loads default font on InitWindow()
extern void
UnloadFontDefault(void); // [Module: text] Unloads default font from GPU memory
#endif

extern int
InitPlatform(void); // Initialize platform (graphics, inputs and more)
extern void ClosePlatform(void); // Close platform

static void InitTimer(void); // Initialize timer, hi-resolution if available
                             // (required by InitPlatform())
static void SetupFramebuffer(
    int width,
    int height); // Setup main framebuffer (required by InitPlatform())
static void
SetupViewport(int width,
              int height); // Set viewport for a provided width and height

static void ScanDirectoryFiles(
    const char *basePath, FilePathList *list,
    const char *filter); // Scan all files and directories in a base path
static void ScanDirectoryFilesRecursively(
    const char *basePath, FilePathList *list,
    const char
        *filter); // Scan all files and directories recursively from a base path

#if defined(SUPPORT_AUTOMATION_EVENTS)
static void
RecordAutomationEvent(void); // Record frame events (to internal events array)
#endif

#if defined(_WIN32) && !defined(PLATFORM_DESKTOP_RGFW)
// NOTE: We declare Sleep() function symbol to avoid including windows.h
// (kernel32.lib linkage required)
__declspec(dllimport) void __stdcall Sleep(
    unsigned long msTimeout); // Required for: WaitTime()
#endif
...
#pragma message(                                                               \
    "WARNING: Enabling SUPPORT_CLIPBOARD_IMAGE requires SUPPORT_MODULE_RTEXTURES to work properly")
#endif

// It's nice to have support Bitmap on Linux as well, but not as necessary as
// Windows
#if !defined(SUPPORT_FILEFORMAT_BMP) && defined(_WIN32)
#pragma message(                                                               \
    "WARNING: Enabling SUPPORT_CLIPBOARD_IMAGE requires SUPPORT_FILEFORMAT_BMP, specially on Windows")
#endif

// From what I've tested applications on Wayland saves images on clipboard as
// PNG
#if (!defined(SUPPORT_FILEFORMAT_PNG) || !defined(SUPPORT_FILEFORMAT_JPG)) &&  \
    !defined(_WIN32)
#pragma message(                                                               \
    "WARNING: Getting image from the clipboard might not work without SUPPORT_FILEFORMAT_PNG or SUPPORT_FILEFORMAT_JPG")
#endif

// Not needed because `rtexture.c` will automatically defined STBI_REQUIRED when
// any SUPPORT_FILEFORMAT_* is defined #if !defined(STBI_REQUIRED)
//     #pragma message ("WARNING: "STBI_REQUIRED is not defined, that means we
//     can't load images from clipbard"
// #endif
...
void InitWindow(int width, int height, const char *title) {
  ...
  CORE.Window.screenScale =
      R_MatrixIdentity(); // No draw scaling required by default
  ...
  // NOTE: CORE.Window.currentFbo.width and CORE.Window.currentFbo.height not
  // used, just stored as globals in rlgl
  ...
  LoadFontDefault();
#if defined(SUPPORT_MODULE_RSHAPES)
  // Set font white rectangle for shapes drawing, so shapes and text can be
  // batched together WARNING: rshapes module is required, if not available,
  // default internal white rectangle is used
  Rectangle rec = GetFontDefault().recs[95];
  if (CORE.Window.flags & FLAG_MSAA_4X_HINT) {
    // NOTE: We try to maxime rec padding to avoid pixel bleeding on MSAA
    // filtering
    SetShapesTexture(GetFontDefault().texture,
                     (Rectangle){rec.x + 2, rec.y + 2, 1, 1});
  } else {
  ...
  SetShapesTexture(
      texture,
      (Rectangle){0.0f, 0.0f, 1.0f, 1.0f}); // WARNING: Module required: rshapes
#endif
#endif
...
void BeginDrawing(void) {
  // WARNING: Previously to BeginDrawing() other render textures drawing could
  // happen, consequently the measure for update vs draw is not accurate (only
  // the total frame time is accurate)
  ...
  // rlTranslatef(0.375, 0.375, 0);    // HACK to have 2D pixel-perfect drawing
  // on OpenGL 1.1
...
#define GIF_RECORD_FRAMERATE 10
    ...
    if (gifFrameCounter > 1000 / GIF_RECORD_FRAMERATE) {
      ...
      unsigned char *screenData =
          rlReadScreenPixels((int)((float)CORE.Window.render.width * scale.x),
...
#define GIF_RECORD_BITRATE 16
      ...
      // Add the frame to the gif recording, given how many frames have passed
      // in centiseconds
      msf_gif_frame(&gifState, screenData, gifFrameCounter / 10,
                    GIF_RECORD_BITRATE,
    ...
    if ((int)(GetTime() / 0.5) % 2 == 1) {
      DrawCircle(30, CORE.Window.screen.height - 20, 10,
                 MAROON); // WARNING: Module required: rshapes
      DrawText("GIF RECORDING", 50, CORE.Window.screen.height - 25, 10,
               RED); // WARNING: Module required: rtext
    }
#endif
  ...
  if (IsKeyPressed(KEY_F12)) {
    ...
    if (IsKeyDown(KEY_LEFT_CONTROL)) {
      if (gifRecording) {
        ...
        SaveFileData(TextFormat("%s/screenrec%03i.gif", CORE.Storage.basePath,
                                screenshotCounter),
                     result.data, (unsigned int)result.dataSize);
        msf_gif_free(result);
...
void EndMode2D(void) {
  ...
  if (rlGetActiveFramebuffer() == 0)
    rlMultMatrixf(MatrixToFloat(
        CORE.Window.screenScale)); // Apply screen scaling if required
}
...
void BeginMode3D(Camera camera) {
  ...
  float aspect = (float)CORE.Window.currentFbo.width /
                 (float)CORE.Window.currentFbo.height;
...
void EndTextureMode(void) {
  ...
  // Go back to the modelview state from BeginDrawing since we are back to the
  // default FBO
  rlMatrixMode(RL_MODELVIEW); // Switch back to modelview matrix
  rlLoadIdentity();           // Reset current matrix (modelview)
  rlMultMatrixf(MatrixToFloat(
...
static void SetScissorRectDPIAware(Rectangle scissorRect) {
  ...
  if (!CORE.Window.usingFbo) {
    ...
    rlScissor(
        ...
        (int)(GetScreenHeight() * scale.y -
              ((scissorRect.y + scissorRect.height) * scale.y)),
  ...
  if (!CORE.Window.usingFbo &&
    ...
    rlScissor(
        ...
        (int)(CORE.Window.currentFbo.height -
              (scissorRect.y + scissorRect.height) * scale.y),
...

{
  ...
  // Counter > MAX_SCISSOR_STACK_SIZE isn't possible without a bug in Raylib,
...

  {
    ...
    TRACELOG(LOG_ERROR,
             "SYSTEM: Scissor stack overflow (MAX_SCISSOR_STACK_SIZE)");
...
// Get active scissor region (or entire render area, if scissor mode not
// enabled)
...

{
  ...
  if (CORE.Window.scissorStackCounter > 0) {
    ...
    return (Rectangle){0, 0, CORE.Window.currentFbo.width,
                       CORE.Window.currentFbo.height};
  }
}
...
void EndScissorMode(void) {
  ...
  // Counter < 0 isn't possible without a bug in Raylib, but check anyway to be
  // safe
...

  {
    ...
    TRACELOG(LOG_ERROR,
             "SYSTEM: Scissor stack empty, are you sure your EndScissorMode "
             "calls match your BeginScissorMode calls?");
  ...
  if (CORE.Window.scissorStackCounter > 0) {
    ...
    Rectangle scissorRect =
        CORE.Window.scissorStack[CORE.Window.scissorStackCounter - 1];
...
VrStereoConfig LoadVrStereoConfig(VrDeviceInfo device) {
  ...
  if (rlGetVersion() != RL_OPENGL_11) {
    ...
    float aspect =
        ((float)device.hResolution * 0.5f) / (float)device.vResolution;

    // Compute lens parameters
    float lensShift =
        (device.hScreenSize * 0.25f - device.lensSeparationDistance * 0.5f) /
        device.hScreenSize;
    ...
    float distortionScale =
        device.lensDistortionValues[0] +
        device.lensDistortionValues[1] * lensRadiusSq +
        device.lensDistortionValues[2] * lensRadiusSq * lensRadiusSq +
        device.lensDistortionValues[3] * lensRadiusSq * lensRadiusSq *
            lensRadiusSq;
    ...
    // Fovy is normally computed with: 2*atan2f(device.vScreenSize,
    // 2*device.eyeToScreenDistance)
    // ...but with lens distortion it is increased (see Oculus SDK
    // Documentation)
    float fovy =
        2.0f *
        atan2f(device.vScreenSize * 0.5f * distortionScale,
               device.eyeToScreenDistance); // Really need distortionScale?
    // float fovy = 2.0f*(float)atan2f(device.vScreenSize*0.5f,
    // device.eyeToScreenDistance);

    // Compute camera projection matrices
    float projOffset =
        4.0f * lensShift; // Scaled to projection space coordinates [-1..1]
    Matrix proj = MatrixPerspective(fovy, aspect, rlGetCullDistanceNear(),
                                    rlGetCullDistanceFar());

    config.projection[0] =
        R_MatrixMultiply(proj, MatrixTranslate(projOffset, 0.0f, 0.0f));
    config.projection[1] =
        R_MatrixMultiply(proj, MatrixTranslate(-projOffset, 0.0f, 0.0f));
    ...
    // some y (base of head to eye level) and -z (center of head to eye
    // protrusion) to the camera positions
    config.viewOffset[0] =
        MatrixTranslate(device.interpupillaryDistance * 0.5f, 0.075f, 0.045f);
    config.viewOffset[1] =
        MatrixTranslate(-device.interpupillaryDistance * 0.5f, 0.075f, 0.045f);
...
Shader LoadShader(const char *vsFileName, const char *fsFileName) {
  ...
  if ((vShaderStr == NULL) && (fShaderStr == NULL))
    TraceLog(
        LOG_WARNING,
        "SHADER: Shader files provided are not valid, using default shader");
...
Shader LoadShaderFromMemory(const char *vsCode, const char *fsCode) {
  ...
  if (shader.id == rlGetShaderIdDefault())
    ...
    shader.locs[SHADER_LOC_VERTEX_COLOR] =
        rlGetLocationAttrib(shader.id, RL_DEFAULT_SHADER_ATTRIB_NAME_COLOR);
    shader.locs[SHADER_LOC_VERTEX_BONEIDS] =
        rlGetLocationAttrib(shader.id, RL_DEFAULT_SHADER_ATTRIB_NAME_BONEIDS);
    shader.locs[SHADER_LOC_VERTEX_BONEWEIGHTS] = rlGetLocationAttrib(
        shader.id, RL_DEFAULT_SHADER_ATTRIB_NAME_BONEWEIGHTS);
    shader.locs[SHADER_LOC_VERTEX_INSTANCE_TX] = rlGetLocationAttrib(
        shader.id, RL_DEFAULT_SHADER_ATTRIB_NAME_INSTANCE_TX);

    // Get handles to GLSL uniform locations (vertex shader)
    shader.locs[SHADER_LOC_MATRIX_MVP] =
        rlGetLocationUniform(shader.id, RL_DEFAULT_SHADER_UNIFORM_NAME_MVP);
    shader.locs[SHADER_LOC_MATRIX_VIEW] =
        rlGetLocationUniform(shader.id, RL_DEFAULT_SHADER_UNIFORM_NAME_VIEW);
    shader.locs[SHADER_LOC_MATRIX_PROJECTION] = rlGetLocationUniform(
        shader.id, RL_DEFAULT_SHADER_UNIFORM_NAME_PROJECTION);
    ...
    shader.locs[SHADER_LOC_BONE_MATRICES] = rlGetLocationUniform(
        shader.id, RL_DEFAULT_SHADER_UNIFORM_NAME_BONE_MATRICES);

    // Get handles to GLSL uniform locations (fragment shader)
    shader.locs[SHADER_LOC_COLOR_DIFFUSE] =
        rlGetLocationUniform(shader.id, RL_DEFAULT_SHADER_UNIFORM_NAME_COLOR);
    shader.locs[SHADER_LOC_MAP_DIFFUSE] = rlGetLocationUniform(
        shader.id,
        RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE0); // SHADER_LOC_MAP_ALBEDO
    shader.locs[SHADER_LOC_MAP_SPECULAR] = rlGetLocationUniform(
        shader.id,
        RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE1); // SHADER_LOC_MAP_METALNESS
    shader.locs[SHADER_LOC_MAP_NORMAL] = rlGetLocationUniform(
        shader.id, RL_DEFAULT_SHADER_SAMPLER2D_NAME_TEXTURE2);
  }
...
bool IsShaderValid(Shader shader) {
  return ((shader.id > 0) && // Validate shader id (GPU loaded successfully)
          (shader.locs != NULL)); // Validate memory has been allocated for
                                  // default shader locations

  // The following locations are tried to be set automatically (locs[i] >= 0),
  // any of them can be checked for validation but the only mandatory one is,
  // afaik, SHADER_LOC_VERTEX_POSITION NOTE: Users can also setup manually their
  // own attributes/uniforms and do not used the default raylib ones

  // Vertex shader attribute locations (default)
  // shader.locs[SHADER_LOC_VERTEX_POSITION]      // Set by default internal
  // shader shader.locs[SHADER_LOC_VERTEX_TEXCOORD01]    // Set by default
  // internal shader shader.locs[SHADER_LOC_VERTEX_TEXCOORD02]
  // shader.locs[SHADER_LOC_VERTEX_NORMAL]
  // shader.locs[SHADER_LOC_VERTEX_TANGENT]
  // shader.locs[SHADER_LOC_VERTEX_COLOR]         // Set by default internal
  // shader

  // Vertex shader uniform locations (default)
  // shader.locs[SHADER_LOC_MATRIX_MVP]           // Set by default internal
  // shader shader.locs[SHADER_LOC_MATRIX_VIEW]
  // shader.locs[SHADER_LOC_MATRIX_PROJECTION]
  // shader.locs[SHADER_LOC_MATRIX_MODEL]
  // shader.locs[SHADER_LOC_MATRIX_NORMAL]

  // Fragment shader uniform locations (default)
  // shader.locs[SHADER_LOC_COLOR_DIFFUSE]        // Set by default internal
  // shader shader.locs[SHADER_LOC_MAP_DIFFUSE]          // Set by default
  // internal shader shader.locs[SHADER_LOC_MAP_SPECULAR]
...
void SetShaderValueV(Shader shader, int locIndex, const void *value,
  ...
  if (locIndex > -1) {
    ...
    // rlDisableShader();      // Avoid resetting current shader program, in
    // case other uniforms are set
  }
}
...
Ray GetScreenToWorldRay(Vector2 position, Camera camera) {
  Ray ray = GetScreenToWorldRayEx(position, camera, GetScreenWidth(),
                                  GetScreenHeight());
...
Ray GetScreenToWorldRayEx(Vector2 position, Camera camera, int width,
                          int height) {
  Ray ray = {0};
  ...
  Matrix matProj = R_MatrixIdentity();

  if (camera.projection == CAMERA_PERSPECTIVE) {
    // Calculate projection matrix from perspective
    matProj = MatrixPerspective(
        camera.fovy * DEG2RAD, ((double)width / (double)height),
...
Matrix GetCameraMatrix2D(Camera2D camera) {
  ...
  //      When setting higher scale, it's more intuitive for the world to become
  //      bigger (= camera become smaller), not for the camera getting bigger,
  //      hence the invert. Same deal with rotation
  //   3. Move it by (-offset);
  //      Offset defines target transform relative to screen, but since we're
  //      effectively "moving" screen (camera) we need to do it into opposite
  //      direction (inverse transform)

  // Having camera transform in world-space, inverse of it gives the modelview
  // transform Since (A*B*C)' = C'*B'*A', the modelview is
  //   1. Move to offset
  //   2. Rotate and Scale
  //   3. Move by -target
  Matrix matOrigin = MatrixTranslate(-camera.target.x, -camera.target.y, 0.0f);
  Matrix matRotation =
      MatrixRotate((Vector3){0.0f, 0.0f, 1.0f}, camera.rotation * DEG2RAD);
  Matrix matScale = MatrixScale(camera.zoom, camera.zoom, 1.0f);
  Matrix matTranslation =
      MatrixTranslate(camera.offset.x, camera.offset.y, 0.0f);

  matTransform = R_MatrixMultiply(
      R_MatrixMultiply(matOrigin, R_MatrixMultiply(matScale, matRotation)),
      matTranslation);
...
Vector2 GetWorldToScreenEx(Vector3 position, Camera camera, int width,
                           int height) {
  ...
  if (camera.projection == CAMERA_PERSPECTIVE) {
    ...
    // Calculate projection matrix from orthographic
    matProj = MatrixOrtho(-right, right, -top, top, rlGetCullDistanceNear(),
                          rlGetCullDistanceFar());
  }
  ...
  Vector3 ndcPos = {worldPos.x / worldPos.w, -worldPos.y / worldPos.w,
                    worldPos.z / worldPos.w};

  // Calculate 2d screen position vector
  Vector2 screenPosition = {(ndcPos.x + 1.0f) / 2.0f * (float)width,
...
Vector2 GetWorldToScreen2D(Vector2 position, Camera2D camera) {
  ...
  Vector3 transform =
      Vector3Transform((Vector3){position.x, position.y, 0}, matCamera);
...
void SetTargetFPS(int fps) {
  ...
  TRACELOG(LOG_INFO, "TIMER: Target time per frame: %02.03f milliseconds",
           (float)CORE.Time.target * 1000.0f);
}
...
// Ref: http://www.geisswerks.com/ryan/FAQS/timing.html --> All about timing on
// Win32!
void WaitTime(double seconds) {
  ...
  double sleepSeconds =
      seconds -
      seconds *
          0.05; // NOTE: We reserve a percentage of the time for busy waiting
#else
...
int GetRandomValue(int min, int max) {
  ...
  // and otherwise if (max - min) > RAND_MAX the random value will incorrectly
  // never exceed a certain threshold NOTE: Depending on the library it can be
  // as low as 32767
  if ((unsigned int)(max - min) > (unsigned int)RAND_MAX) {
    TRACELOG(LOG_WARNING,
             "Invalid GetRandomValue() arguments, range should not be higher "
             "than %i",
             RAND_MAX);
  }
...
void TakeScreenshot(const char *fileName) {
  ...
  if (strchr(fileName, '\'') != NULL) {
    TRACELOG(LOG_WARNING, "SYSTEM: Provided fileName could be potentially "
  ...
  Image image = {imgData, (int)((float)CORE.Window.render.width * scale.x),
                 (int)((float)CORE.Window.render.height * scale.y), 1,
                 PIXELFORMAT_UNCOMPRESSED_R8G8B8A8};
...
void SetConfigFlags(unsigned int flags) {
  if (CORE.Window.ready)
    TRACELOG(LOG_WARNING,
             "WINDOW: SetConfigFlags called after window initialization, Use "
             "\"SetWindowState\" to set flags instead");
...
#define MAX_FILE_EXTENSION_LENGTH 16
  ...
  if (fileExt != NULL) {
    ...
    int extCount = 0;
    char **checkExts =
        TextSplit(ext, ';', &extCount); // WARNING: Module required: rtext

    char fileExtLower[MAX_FILE_EXTENSION_LENGTH + 1] = {0};
    strncpy(fileExtLower, TextToLower(fileExt),
            MAX_FILE_EXTENSION_LENGTH); // WARNING: Module required: rtext
...
int GetFileLength(const char *fileName) {
  ...
  if (file != NULL) {
    ...
    if (fileSize > 2147483647)
      TRACELOG(
          LOG_WARNING,
          "[%s] File size overflows expected limit, do not use GetFileLength()",
          fileName);
    else
      size = (int)fileSize;
...
const char *GetDirectoryPath(const char *filePath) {
  ...
  // fortunately, Windows also support the '/' separator, that's the one should
  be used #if defined(_WIN32) char separator = '\\'; #else char separator = '/';
  #endif
  */
  ...
  // In case provided path does not contain a root drive letter (C:\, D:\) nor
  // leading path separator (\, /), we add the current directory path to dirPath
...
FilePathList LoadDirectoryFiles(const char *dirPath) {
  ...
  if (dir != NULL) // It's a directory
  {
    ...
    TRACELOG(
        LOG_WARNING,
        "FILEIO: Failed to open requested directory"); // Maybe it's a file...

  return files;
}

// Load directory filepaths with extension filtering and recursive directory
// scan NOTE: On recursive loading we do not pre-scan for file count, we use
...
int MakeDirectory(const char *dirPath) {
  ...
  if (DirectoryExists(dirPath))
    return 0; // Path already exists (is valid)
...
bool ChangeDirectory(const char *dir) {
  ...
  if (result != 0)
    ...
    TRACELOG(LOG_INFO, "SYSTEM: Working Directory: %s", dir);
...
bool IsFileNameValid(const char *fileName) {
  ...
  if ((fileName != NULL) && (fileName[0] != '\0')) {
    ...
    /*
       ...
       (fileName[2] == 'N')) ||   // CON
                        ((fileName[0] == 'P') && (fileName[1] == 'R') &&
       (fileName[2] == 'N')) ||   // PRN
                        ((fileName[0] == 'A') && (fileName[1] == 'U') &&
       (fileName[2] == 'X')) ||   // AUX
                        ((fileName[0] == 'N') && (fileName[1] == 'U') &&
       (fileName[2] == 'L'))) valid = false; // NUL
                }

                if (length >= 4)
                {
                    if (((fileName[0] == 'C') && (fileName[1] == 'O') &&
       (fileName[2] == 'M') && ((fileName[3] >= '0') && (fileName[3] <= '9')))
       ||  // COM0-9
                        ((fileName[0] == 'L') && (fileName[1] == 'P') &&
       (fileName[2] == 'T') && ((fileName[3] >= '0') && (fileName[3] <= '9'))))
       valid = false; // LPT0-9
                }
            }
    */
...
#define COMPRESSION_QUALITY_DEFLATE 8
  ...
  struct sdefl *sdefl = (struct sdefl *)RL_CALLOC(
      1, sizeof(struct sdefl)); // WARNING: Possible stack overflow, struct
                                // sdefl is almost 1MB
  int bounds = sdefl_bound(dataSize);
  compData = (unsigned char *)RL_CALLOC(bounds, 1);

  *compDataSize = sdeflate(
      sdefl, compData, data, dataSize,
      COMPRESSION_QUALITY_DEFLATE); // Compression level 8, same as stbiw
  RL_FREE(sdefl);

  TRACELOG(LOG_INFO,
           "SYSTEM: Compress data: Original size: %i -> Comp. size: %i",
           dataSize, *compDataSize);
#endif
...
unsigned char *DecompressData(const unsigned char *compData, int compDataSize,
                              int *dataSize) {
  ...
  int length = sinflate(data, MAX_DECOMPRESSION_SIZE * 1024 * 1024, compData,
                        compDataSize);

  // WARNING: RL_REALLOC can make (and leave) data copies in memory, be careful
  // with sensitive compressed data!
  // TODO: Use a different approach, create another buffer, copy data manually
  // to it and wipe original buffer memory
  ...
  *dataSize = length;
  ...
  TRACELOG(LOG_INFO,
           "SYSTEM: Decompress data: Comp. size: %i -> Original size: %i",
...
char *EncodeDataBase64(const unsigned char *data, int dataSize,
                       int *outputSize) {
  // Base64 conversion table from RFC 4648 [0..63]
  // NOTE: They represent 64 values (6 bits), to encode 3 bytes of data into 4
  // "sixtets" (6bit characters)
  ...
  while (paddedSize % 3 != 0)
    paddedSize++; // Padding bytes to round 4*(dataSize/3) to 4 bytes
  ...
  for (int i = 0; i < dataSize;) {
    ...
    encodedData[outputCount + 0] =
        (unsigned char)(base64EncodeTable[(octetPack >> 18) & 0x3f]);
    encodedData[outputCount + 1] =
        (unsigned char)(base64EncodeTable[(octetPack >> 12) & 0x3f]);
    encodedData[outputCount + 2] =
        (unsigned char)(base64EncodeTable[(octetPack >> 6) & 0x3f]);
    encodedData[outputCount + 3] =
        (unsigned char)(base64EncodeTable[octetPack & 0x3f]);
    outputCount += 4;
    i += 3;
  }
  ...
  for (int p = 0; p < padding; p++)
    encodedData[outputCount - p - 1] = '=';

  // Add null terminator to string
  encodedData[outputCount] = '\0';
  outputCount++;

  if (outputCount != estimatedOutputSize)
    TRACELOG(LOG_WARNING, "BASE64: Output size differs from estimation");
...
unsigned char *DecodeDataBase64(const char *text, int *outputSize) {
  ...
  // Compute expected size and padding
  int dataSize =
      (int)strlen(text); // WARNING: Expecting NULL terminated strings!
  ...
  for (int i = 0; i < dataSize;) {
    ...
    unsigned int sixtetC = ((unsigned char)text[i + 2] != '=')
                               ? base64DecodeTable[(unsigned char)text[i + 2]]
                               : 0;
    unsigned int sixtetD = ((unsigned char)text[i + 3] != '=')
                               ? base64DecodeTable[(unsigned char)text[i + 3]]
                               : 0;

    unsigned int octetPack =
        (sixtetA << 18) | (sixtetB << 12) | (sixtetC << 6) | sixtetD;
  ...
  if (estimatedOutputSize != (outputCount - padding))
    TRACELOG(LOG_WARNING, "BASE64: Decoded size differs from estimation");
...
#define ROTATE_LEFT(x, c) (((x) << (c)) | ((x) >> (32 - (c))))
  ...
  // WARNING: All variables are unsigned 32 bit and wrap modulo 2^32 when
  // calculating

  // NOTE: r specifies the per-round shift amounts
  unsigned int r[] = {7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 7,
                      12, 17, 22, 5,  9,  14, 20, 5,  9,  14, 20, 5,  9,
                      14, 20, 5,  9,  14, 20, 4,  11, 16, 23, 4,  11, 16,
                      23, 4,  11, 16, 23, 4,  11, 16, 23, 6,  10, 15, 21,
                      6,  10, 15, 21, 6,  10, 15, 21, 6,  10, 15, 21};

  // Using binary integer part of the sines of integers (in radians) as
  // constants
  unsigned int k[] = {
  ...
  unsigned char *msg = (unsigned char *)RL_CALLOC(
      newDataSize + 64,
      1); // Initialize with '0' bits, allocating 64 extra bytes
  memcpy(msg, data, dataSize);
  msg[dataSize] = 128; // Write the '1' bit

  unsigned int bitsLen = 8 * dataSize;
  memcpy(msg + newDataSize, &bitsLen,
         4); // Append the len in bits at the end of the buffer

  // Process the message in successive 512-bit chunks for each 512-bit chunk of
  // message
...
#define ROTATE_LEFT(x, c) (((x) << (c)) | ((x) >> (32 - (c))))
  ...
  unsigned char *msg =
      (unsigned char *)RL_CALLOC(newDataSize, 1); // Initialize with '0' bits
  memcpy(msg, data, dataSize);
  msg[dataSize] = 128; // Write the '1' bit
  ...
  for (int offset = 0; offset < newDataSize; offset += (512 / 8)) {
    ...
    // Message schedule: extend the sixteen 32-bit words into eighty 32-bit
    // words:
    for (int i = 16; i < 80; i++)
...
AutomationEventList LoadAutomationEventList(const char *fileName) {
  ...
  list.events = (AutomationEvent *)RL_CALLOC(MAX_AUTOMATION_EVENTS,
  ...
  if (fileName == NULL)
    ...
    if ((fileId[0] == 'r') && (fileId[1] == 'A') && (fileId[2] == 'E') &&
    (fileId[1] == ' '))
    {
    ...
    */
    ...
    if (raeFile != NULL) {
      ...
      while (!feof(raeFile)) {
        ...
        case 'e': {
          sscanf(buffer, "e %d %d %d %d %d %d %[^\n]s",
                 &list.events[counter].frame, &list.events[counter].type,
                 &list.events[counter].params[0],
                 &list.events[counter].params[1],
                 &list.events[counter].params[2],
                 &list.events[counter].params[3], eventDesc);
      ...
      if (counter != list.count) {
        TRACELOG(LOG_WARNING,
                 "AUTOMATION: Events read from file [%i] do not mach event "
                 "count specified [%i]",
                 counter, list.count);
        list.count = counter;
      }
...
bool ExportAutomationEventList(AutomationEventList list, const char *fileName) {
  ...
  */
  ...
  char *txtData = (char *)RL_CALLOC(
      256 * list.count + 2048,
      sizeof(char)); // 256 characters per line plus some header
  ...
  byteCount += sprintf(txtData + byteCount,
                       "#    e <frame> <event_type> <param0> <param1> <param2> "
                       "<param3> // <event_type_name>\n");
  byteCount += sprintf(txtData + byteCount, "#\n");
  byteCount +=
  ...
  for (unsigned int i = 0; i < list.count; i++) {
    byteCount += snprintf(
        txtData + byteCount, 256, "e %i %i %i %i %i %i // Event: %s\n",
        list.events[i].frame, list.events[i].type, list.events[i].params[0],
        list.events[i].params[1], list.events[i].params[2],
        list.events[i].params[3], autoEventTypeName[list.events[i].type]);
  }
...
void SetAutomationEventBaseFrame(int frame) { CORE.Time.frameCounter = frame; }
...
void PlayAutomationEvent(AutomationEvent event) {
#if defined(SUPPORT_AUTOMATION_EVENTS)
  // WARNING: When should event be played? After/before/replace
  // PollInputEvents()? -> Up to the user!
  ...
  if (!automationEventRecording) // TODO: Allow recording events while playing?
  {
    ...
    case INPUT_GAMEPAD_BUTTON_UP:
      CORE.Input.Gamepad.currentButtonState[event.params[0]][event.params[1]] =
          false;
    ...
    case INPUT_GAMEPAD_BUTTON_DOWN:
      CORE.Input.Gamepad.currentButtonState[event.params[0]][event.params[1]] =
          true;
      break;                        // param[0]: gamepad, param[1]: button
    case INPUT_GAMEPAD_AXIS_MOTION: // param[0]: gamepad, param[1]: axis,
                                    // param[2]: delta
    {
      CORE.Input.Gamepad.axisState[event.params[0]][event.params[1]] =
          ((float)event.params[2] / 32768.0f);
    } break;
#if defined(SUPPORT_GESTURES_SYSTEM)
    case INPUT_GESTURE:
      GESTURES.current = event.params[0];
      break; // param[0]: gesture (enum Gesture) -> rgestures.h:
             // GESTURES.current
#endif
    // Window event
    case WINDOW_CLOSE:
    ...
    TRACELOG(LOG_INFO,
             "AUTOMATION PLAY: Frame: %i | Event type: %i | Event parameters: "
             "%i, %i, %i",
             event.frame, event.type, event.params[0], event.params[1],
             event.params[2]);
  }
#endif
}
...
bool IsKeyPressed(int key) {
  ...
  if ((key > 0) && (key < MAX_KEYBOARD_KEYS)) {
    if ((CORE.Input.Keyboard.previousKeyState[key] == 0) &&
...
bool IsKeyPressedRepeat(int key) {
  ...
  if ((key > 0) && (key < MAX_KEYBOARD_KEYS)) {
    if (CORE.Input.Keyboard.keyRepeatInFrame[key] == 1)
      repeat = true;
  }
...
bool IsKeyReleased(int key) {
  ...
  if ((key > 0) && (key < MAX_KEYBOARD_KEYS)) {
    if ((CORE.Input.Keyboard.previousKeyState[key] == 1) &&
...
int GetKeyPressed(void) {
  ...
  if (CORE.Input.Keyboard.keyPressedQueueCount > 0) {
    ...
    for (int i = 0; i < (CORE.Input.Keyboard.keyPressedQueueCount - 1); i++)
      CORE.Input.Keyboard.keyPressedQueue[i] =
          CORE.Input.Keyboard.keyPressedQueue[i + 1];

    // Reset last character in the queue
    CORE.Input.Keyboard
        .keyPressedQueue[CORE.Input.Keyboard.keyPressedQueueCount - 1] = 0;
    CORE.Input.Keyboard.keyPressedQueueCount--;
  }
...
int GetCharPressed(void) {
  ...
  if (CORE.Input.Keyboard.charPressedQueueCount > 0) {
    ...
    for (int i = 0; i < (CORE.Input.Keyboard.charPressedQueueCount - 1); i++)
      CORE.Input.Keyboard.charPressedQueue[i] =
          CORE.Input.Keyboard.charPressedQueue[i + 1];

    // Reset last character in the queue
    CORE.Input.Keyboard
        .charPressedQueue[CORE.Input.Keyboard.charPressedQueueCount - 1] = 0;
...
bool IsGamepadAvailable(int gamepad) {
  ...
  if ((gamepad < MAX_GAMEPADS) && CORE.Input.Gamepad.ready[gamepad])
    result = true;
...
float GetGamepadAxisMovement(int gamepad, int axis) {
  float value = ((axis == GAMEPAD_AXIS_LEFT_TRIGGER) ||
  ...
  if ((gamepad < MAX_GAMEPADS) && CORE.Input.Gamepad.ready[gamepad] &&
    ...
    float movement = (value < 0.0f)
                         ? CORE.Input.Gamepad.axisState[gamepad][axis]
...
bool IsMouseButtonPressed(int button) {
  ...
  if ((CORE.Input.Mouse.currentButtonState[button] == 1) &&
      (CORE.Input.Mouse.previousButtonState[button] == 0))
    pressed = true;

  // Map touches to mouse buttons checking
  if ((CORE.Input.Touch.currentTouchState[button] == 1) &&
      (CORE.Input.Touch.previousTouchState[button] == 0))
    pressed = true;
...
bool IsMouseButtonReleased(int button) {
  ...
  if ((CORE.Input.Mouse.currentButtonState[button] == 0) &&
      (CORE.Input.Mouse.previousButtonState[button] == 1))
    released = true;

  // Map touches to mouse buttons checking
  if ((CORE.Input.Touch.currentTouchState[button] == 0) &&
      (CORE.Input.Touch.previousTouchState[button] == 1))
    released = true;
...
int GetMouseX(void) {
  int mouseX =
      (int)((CORE.Input.Mouse.currentPosition.x + CORE.Input.Mouse.offset.x) *
...
int GetMouseY(void) {
  int mouseY =
      (int)((CORE.Input.Mouse.currentPosition.y + CORE.Input.Mouse.offset.y) *
...
Vector2 GetMousePosition(void) {
  ...
  position.x =
      (CORE.Input.Mouse.currentPosition.x + CORE.Input.Mouse.offset.x) *
      CORE.Input.Mouse.scale.x;
  position.y =
      (CORE.Input.Mouse.currentPosition.y + CORE.Input.Mouse.offset.y) *
      CORE.Input.Mouse.scale.y;
...
Vector2 GetMouseDelta(void) {
  ...
  delta.x =
      CORE.Input.Mouse.currentPosition.x - CORE.Input.Mouse.previousPosition.x;
  delta.y =
...
float GetMouseWheelMove(void) {
  ...
  if (fabsf(CORE.Input.Mouse.currentWheelMove.x) >
      fabsf(CORE.Input.Mouse.currentWheelMove.y))
...
// TODO: Touch position should be scaled depending on display size and render
// size
Vector2 GetTouchPosition(int index) {
  ...
  if (index < MAX_TOUCH_POINTS)
    ...
    TRACELOG(LOG_WARNING,
             "INPUT: Required touch point out of range (Max touch points: %i)",
             MAX_TOUCH_POINTS);
...
int GetTouchPointId(int index) {
  ...
  if (index < MAX_TOUCH_POINTS)
    id = CORE.Input.Touch.pointId[index];
...
void InitTimer(void) {
  ...
  // high-resolution performance counter
    ...
    !defined(SUPPORT_BUSY_WAIT_LOOP) && !defined(PLATFORM_DESKTOP_SDL)
  timeBeginPeriod(
      1); // Setup high-resolution timer to 1ms (granularity of 1-2 ms)
#endif
  ...
  {
    CORE.Time.base = (unsigned long long int)now.tv_sec * 1000000000LLU +
...
void SetupViewport(int width, int height) {
  ...
  // NOTE: We consider render size (scaled) and offset in case black bars are
  // required and render area does not match full display area (this situation
  // is only applicable on fullscreen mode)
#if defined(__APPLE__)
  Vector2 scale = GetWindowScaleDPI();
  rlViewport(CORE.Window.renderOffset.x / 2 * scale.x,
             CORE.Window.renderOffset.y / 2 * scale.y,
             (CORE.Window.render.width) * scale.x,
             (CORE.Window.render.height) * scale.y);
#else
  rlViewport(CORE.Window.renderOffset.x / 2, CORE.Window.renderOffset.y / 2,
  ...
  // Set orthographic projection to current framebuffer size
  // NOTE: Configured top-left corner as (0, 0)
  rlOrtho(0, CORE.Window.render.width, CORE.Window.render.height, 0, 0.0f,
          1.0f);
...
void SetupFramebuffer(int width, int height) {
  // Calculate CORE.Window.render.width and CORE.Window.render.height, we have
  // the display size (input params) and the desired screen size (global var)
  if ((CORE.Window.screen.width > CORE.Window.display.width) ||
      (CORE.Window.screen.height > CORE.Window.display.height)) {
    TRACELOG(LOG_WARNING,
             "DISPLAY: Downscaling required: Screen size (%ix%i) is bigger "
             "than display size (%ix%i)",
             CORE.Window.screen.width, CORE.Window.screen.height,
             CORE.Window.display.width, CORE.Window.display.height);

    // Downscaling to fit display with border-bars
    float widthRatio =
        (float)CORE.Window.display.width / (float)CORE.Window.screen.width;
    float heightRatio =
        (float)CORE.Window.display.height / (float)CORE.Window.screen.height;

    if (widthRatio <= heightRatio) {
      CORE.Window.render.width = CORE.Window.display.width;
      CORE.Window.render.height =
          (int)round((float)CORE.Window.screen.height * widthRatio);
      CORE.Window.renderOffset.x = 0;
      CORE.Window.renderOffset.y =
          (CORE.Window.display.height - CORE.Window.render.height);
    } else {
      CORE.Window.render.width =
          (int)round((float)CORE.Window.screen.width * heightRatio);
      CORE.Window.render.height = CORE.Window.display.height;
      CORE.Window.renderOffset.x =
          (CORE.Window.display.width - CORE.Window.render.width);
      CORE.Window.renderOffset.y = 0;
    }

    // Screen scaling required
    float scaleRatio =
        (float)CORE.Window.render.width / (float)CORE.Window.screen.width;
    CORE.Window.screenScale = MatrixScale(scaleRatio, scaleRatio, 1.0f);

    // NOTE: We render to full display resolution!
    // We just need to calculate above parameters for downscale matrix and
    // offsets
    CORE.Window.render.width = CORE.Window.display.width;
    CORE.Window.render.height = CORE.Window.display.height;

    TRACELOG(LOG_WARNING,
             "DISPLAY: Downscale matrix generated, content will be rendered at "
             "(%ix%i)",
             CORE.Window.render.width, CORE.Window.render.height);
    ...
    TRACELOG(LOG_INFO,
             "DISPLAY: Upscaling required: Screen size (%ix%i) smaller than "
             "display size (%ix%i)",
             CORE.Window.screen.width, CORE.Window.screen.height,
             CORE.Window.display.width, CORE.Window.display.height);
    ...
    float displayRatio =
        (float)CORE.Window.display.width / (float)CORE.Window.display.height;
    float screenRatio =
        (float)CORE.Window.screen.width / (float)CORE.Window.screen.height;

    if (displayRatio <= screenRatio) {
      CORE.Window.render.width = CORE.Window.screen.width;
      CORE.Window.render.height =
          (int)round((float)CORE.Window.screen.width / displayRatio);
      CORE.Window.renderOffset.x = 0;
      CORE.Window.renderOffset.y =
          (CORE.Window.render.height - CORE.Window.screen.height);
    } else {
      CORE.Window.render.width =
          (int)round((float)CORE.Window.screen.height * displayRatio);
      CORE.Window.render.height = CORE.Window.screen.height;
      CORE.Window.renderOffset.x =
          (CORE.Window.render.width - CORE.Window.screen.width);
      CORE.Window.renderOffset.y = 0;
...
static void ScanDirectoryFiles(const char *basePath, FilePathList *files,
                               const char *filter) {
    ...
    while ((dp = readdir(dir)) != NULL) {
      if ((strcmp(dp->d_name, ".") != 0) && (strcmp(dp->d_name, "..") != 0)) {
#if defined(_WIN32)
        int pathLength = snprintf(path, MAX_FILEPATH_LENGTH - 1, "%s\\%s",
                                  basePath, dp->d_name);
#else
        int pathLength = snprintf(path, MAX_FILEPATH_LENGTH - 1, "%s/%s",
                                  basePath, dp->d_name);
        ...
        if ((pathLength < 0) || (pathLength >= MAX_FILEPATH_LENGTH)) {
          TRACELOG(LOG_WARNING,
                   "FILEIO: Path longer than %d characters (%s...)",
                   MAX_FILEPATH_LENGTH, basePath);
        } else if (filter != NULL) {
          if (IsPathFile(path)) {
...
static void ScanDirectoryFilesRecursively(const char *basePath,
                                          FilePathList *files,
                                          const char *filter) {
  // WARNING: Path can not be static or it will be reused between recursive
  // function calls!
    ...
    while (((dp = readdir(dir)) != NULL) && (files->count < files->capacity)) {
      if ((strcmp(dp->d_name, ".") != 0) && (strcmp(dp->d_name, "..") != 0)) {
        ...
        if ((pathLength < 0) || (pathLength >= MAX_FILEPATH_LENGTH)) {
          ...
          if ((filter != NULL) &&
              (strstr(filter, DIRECTORY_FILTER_TAG) != NULL)) {
            strcpy(files->paths[files->count], path);
            files->count++;
          }
...
static void RecordAutomationEvent(void) {
  ...
  for (int key = 0; key < MAX_KEYBOARD_KEYS; key++) {
    ...
    if (CORE.Input.Keyboard.previousKeyState[key] &&
      ...
      TRACELOG(LOG_INFO,
               "AUTOMATION: Frame: %i | Event type: INPUT_KEY_UP | Event "
               "parameters: %i, %i, %i",
               currentEventList->events[currentEventList->count].frame,
    ...
    if (CORE.Input.Keyboard.currentKeyState[key]) {
      ...
      TRACELOG(LOG_INFO,
               "AUTOMATION: Frame: %i | Event type: INPUT_KEY_DOWN | Event "
               "parameters: %i, %i, %i",
               currentEventList->events[currentEventList->count].frame,
               currentEventList->events[currentEventList->count].params[0],
               currentEventList->events[currentEventList->count].params[1],
  ...
  for (int button = 0; button < MAX_MOUSE_BUTTONS; button++) {
    ...
    if (CORE.Input.Mouse.previousButtonState[button] &&
        !CORE.Input.Mouse.currentButtonState[button]) {
      currentEventList->events[currentEventList->count].frame =
          CORE.Time.frameCounter;
      currentEventList->events[currentEventList->count].type =
          INPUT_MOUSE_BUTTON_UP;
      ...
      TRACELOG(LOG_INFO,
               "AUTOMATION: Frame: %i | Event type: INPUT_MOUSE_BUTTON_UP | "
               "Event parameters: %i, %i, %i",
               currentEventList->events[currentEventList->count].frame,
               currentEventList->events[currentEventList->count].params[0],
               currentEventList->events[currentEventList->count].params[1],
               currentEventList->events[currentEventList->count].params[2]);
    ...
    if (CORE.Input.Mouse.currentButtonState[button]) {
      ...
      TRACELOG(LOG_INFO,
               "AUTOMATION: Frame: %i | Event type: INPUT_MOUSE_BUTTON_DOWN | "
               "Event parameters: %i, %i, %i",
               currentEventList->events[currentEventList->count].frame,
               currentEventList->events[currentEventList->count].params[0],
               currentEventList->events[currentEventList->count].params[1],
  ...
  if (((int)CORE.Input.Mouse.currentPosition.x !=
       (int)CORE.Input.Mouse.previousPosition.x) ||
      ((int)CORE.Input.Mouse.currentPosition.y !=
       (int)CORE.Input.Mouse.previousPosition.y)) {
    currentEventList->events[currentEventList->count].frame =
        CORE.Time.frameCounter;
    currentEventList->events[currentEventList->count].type =
        INPUT_MOUSE_POSITION;
    currentEventList->events[currentEventList->count].params[0] =
        (int)CORE.Input.Mouse.currentPosition.x;
    currentEventList->events[currentEventList->count].params[1] =
        (int)CORE.Input.Mouse.currentPosition.y;
    currentEventList->events[currentEventList->count].params[2] = 0;

    TRACELOG(LOG_INFO,
             "AUTOMATION: Frame: %i | Event type: INPUT_MOUSE_POSITION | Event "
             "parameters: %i, %i, %i",
             currentEventList->events[currentEventList->count].frame,
             currentEventList->events[currentEventList->count].params[0],
             currentEventList->events[currentEventList->count].params[1],
  ...
  if (((int)CORE.Input.Mouse.currentWheelMove.x !=
       (int)CORE.Input.Mouse.previousWheelMove.x) ||
      ((int)CORE.Input.Mouse.currentWheelMove.y !=
       (int)CORE.Input.Mouse.previousWheelMove.y)) {
    currentEventList->events[currentEventList->count].frame =
        CORE.Time.frameCounter;
    currentEventList->events[currentEventList->count].type =
        INPUT_MOUSE_WHEEL_MOTION;
    currentEventList->events[currentEventList->count].params[0] =
        (int)CORE.Input.Mouse.currentWheelMove.x;
    currentEventList->events[currentEventList->count].params[1] =
        (int)CORE.Input.Mouse.currentWheelMove.y;
    currentEventList->events[currentEventList->count].params[2] = 0;

    TRACELOG(LOG_INFO,
             "AUTOMATION: Frame: %i | Event type: INPUT_MOUSE_WHEEL_MOTION | "
             "Event parameters: %i, %i, %i",
             currentEventList->events[currentEventList->count].frame,
             currentEventList->events[currentEventList->count].params[0],
             currentEventList->events[currentEventList->count].params[1],
  ...
  for (int id = 0; id < MAX_TOUCH_POINTS; id++) {
    ...
    if (CORE.Input.Touch.previousTouchState[id] &&
        !CORE.Input.Touch.currentTouchState[id]) {
      currentEventList->events[currentEventList->count].frame =
          CORE.Time.frameCounter;
      ...
      TRACELOG(LOG_INFO,
               "AUTOMATION: Frame: %i | Event type: INPUT_TOUCH_UP | Event "
               "parameters: %i, %i, %i",
               currentEventList->events[currentEventList->count].frame,
               currentEventList->events[currentEventList->count].params[0],
               currentEventList->events[currentEventList->count].params[1],
    ...
    if (CORE.Input.Touch.currentTouchState[id]) {
      ...
      TRACELOG(LOG_INFO,
               "AUTOMATION: Frame: %i | Event type: INPUT_TOUCH_DOWN | Event "
               "parameters: %i, %i, %i",
               currentEventList->events[currentEventList->count].frame,
               currentEventList->events[currentEventList->count].params[0],
               currentEventList->events[currentEventList->count].params[1],
    ...
    /*
    if (((int)CORE.Input.Touch.currentPosition[id].x !=
    (int)CORE.Input.Touch.previousPosition[id].x) ||
        ((int)CORE.Input.Touch.currentPosition[id].y !=
    (int)CORE.Input.Touch.previousPosition[id].y))
    {
        currentEventList->events[currentEventList->count].frame =
    CORE.Time.frameCounter;
        currentEventList->events[currentEventList->count].type =
    INPUT_TOUCH_POSITION;
        currentEventList->events[currentEventList->count].params[0] = id;
        currentEventList->events[currentEventList->count].params[1] =
    (int)CORE.Input.Touch.currentPosition[id].x;
        currentEventList->events[currentEventList->count].params[2] =
    (int)CORE.Input.Touch.currentPosition[id].y;

        TRACELOG(LOG_INFO, "AUTOMATION: Frame: %i | Event type:
    INPUT_TOUCH_POSITION | Event parameters: %i, %i, %i",
    currentEventList->events[currentEventList->count].frame,
    currentEventList->events[currentEventList->count].params[0],
    currentEventList->events[currentEventList->count].params[1],
    currentEventList->events[currentEventList->count].params[2]);
  ...
  for (int gamepad = 0; gamepad < MAX_GAMEPADS; gamepad++) {
    ...
    /*
    if ((CORE.Input.Gamepad.currentState[gamepad] !=
    CORE.Input.Gamepad.previousState[gamepad]) &&
    ...
    */
    ...
    if ((CORE.Input.Gamepad.currentState[gamepad] !=
    CORE.Input.Gamepad.previousState[gamepad]) &&
        (!CORE.Input.Gamepad.currentState[gamepad])) // Check if changed to
    not-ready
    {
    ...
    */
    ...
    for (int button = 0; button < MAX_GAMEPAD_BUTTONS; button++) {
      ...
      if (CORE.Input.Gamepad.currentButtonState[gamepad][button]) {
        ...
        TRACELOG(LOG_INFO,
                 "AUTOMATION: Frame: %i | Event type: "
                 "INPUT_GAMEPAD_BUTTON_DOWN | Event parameters: %i, %i, %i",
                 currentEventList->events[currentEventList->count].frame,
                 currentEventList->events[currentEventList->count].params[0],
                 currentEventList->events[currentEventList->count].params[1],
                 currentEventList->events[currentEventList->count].params[2]);
    ...
    for (int axis = 0; axis < MAX_GAMEPAD_AXES; axis++) {
      ...
      float defaultMovement = ((axis == GAMEPAD_AXIS_LEFT_TRIGGER) ||
                               (axis == GAMEPAD_AXIS_RIGHT_TRIGGER))
                                  ? -1.0f
                                  : 0.0f;
      if (GetGamepadAxisMovement(gamepad, axis) != defaultMovement) {
        ...
        TRACELOG(LOG_INFO,
                 "AUTOMATION: Frame: %i | Event type: "
                 "INPUT_GAMEPAD_AXIS_MOTION | Event parameters: %i, %i, %i",
                 currentEventList->events[currentEventList->count].frame,
                 currentEventList->events[currentEventList->count].params[0],
                 currentEventList->events[currentEventList->count].params[1],
                 currentEventList->events[currentEventList->count].params[2]);
  ...
  if (GESTURES.current != GESTURE_NONE) {
    ...
    TRACELOG(LOG_INFO,
             "AUTOMATION: Frame: %i | Event type: INPUT_GESTURE | Event "
             "parameters: %i, %i, %i",
             currentEventList->events[currentEventList->count].frame,
             currentEventList->events[currentEventList->count].params[0],
             currentEventList->events[currentEventList->count].params[1],
