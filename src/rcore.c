...
typedef struct CoreData {
    struct {
        ...
        Rectangle scissorStack[MAX_SCISSOR_STACK_SIZE];// Scissor stack for nested scissor callsAdd commentMore actions
...
void InitWindow(int width, int height, const char *title)
{
    ...
    CORE.Window.screenScale = R_MatrixIdentity();     // No draw scaling required by default
...
VrStereoConfig LoadVrStereoConfig(VrDeviceInfo device)
{
    ...
    if (rlGetVersion() != RL_OPENGL_11)
    {
       ...
       // float fovy = 2.0f*(float)atan2f(device.vScreenSize*0.5f, device.eyeToScreenDistance);
        ...
        config.projection[0] = R_MatrixMultiply(proj, MatrixTranslate(projOffset, 0.0f, 0.0f));
        config.projection[1] = R_MatrixMultiply(proj, MatrixTranslate(-projOffset, 0.0f, 0.0f));
...
Matrix GetCameraMatrix2D(Camera2D camera)
{
    ...
    matTransform = R_MatrixMultiply(R_MatrixMultiply(matOrigin, R_MatrixMultiply(matScale, matRotation)), matTranslation);
